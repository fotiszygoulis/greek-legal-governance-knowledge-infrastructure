# Law 4858/2021 Working Extraction v1

## Document Identity
- Document ID: DOC-002
- Title: Law 4858/2021 Cultural Heritage Protection Code
- Working file: 2021_law_4858_cultural_heritage_protection_code.pdf.pdf
- Corpus folder: 01_core_normative_corpus

## Extraction Objective
This working note supports the first document-level decoding of Law 4858/2021 for the AGRIR-Lab component **Greek Legal-Governance Knowledge Infrastructure for AI and Institutional Research**.

## First Extraction Targets
- legal definitions of cultural heritage and monuments
- immovable monuments
- movable monuments
- archaeological sites and protection zones
- archaeological research
- museums
- access, use, imaging, and permissions relevance
- exports, returns, and illicit trafficking relevance
- collective bodies relevance
- criminal protection relevance

## Coding Relevance
Primary coding categories expected:
- competence
- supervision
- permit_or_approval
- museum_governance
- archive_and_documentation
- anti_trafficking_and_protection
- transition_provision

## Working Status
- decoding: not started
- coding: not started
- mapping: not started

## Note
This is the second document-level working extraction file and will be expanded gradually.
## First Decoding Pass

### A. Article-Level Priority Nodes
- Article 1: Object of protection
- Article 2: Key legal definitions
- Article 3: Content of protection
- Article 4: National Archive of Monuments
- Article 6: Distinctions of immovable monuments
- Article 10: Actions affecting immovable monuments and their environment
- Articles 12-17A: Archaeological sites and protection zones
- Article 20: Distinctions of movable monuments
- Articles 31-34: Collectors, dealers, import/export and returns
- Articles 35-44: Archaeological research and monument protection works
- Article 45: Museums
- Article 46: Access and use of monuments and spaces
- Articles 49-52: Collective bodies
- Articles 53-72: Criminal protection

### B. First Legal-Structural Extraction
#### Articles 1-3 — Object, Definitions, and Content of Protection
Initial decoding focus:
- scope of cultural heritage
- tangible and intangible heritage
- historical memory
- public interest logic
- documentation, study, preservation, access, and public awareness

#### Article 4 — National Archive of Monuments
Initial decoding focus:
- monument registration
- documentation obligation
- archive governance
- access conditions
- data protection relevance
- inspection reporting relevance

#### Article 6 — Immovable Monuments
Initial decoding focus:
- legal distinctions of immovable monuments
- automatic legal protection
- characterization process
- ministerial decision relevance
- council opinion relevance
- protection effects before final characterization

#### Article 10 — Actions Affecting Monuments
Initial decoding focus:
- prohibition logic
- ministerial approval requirement
- intervention thresholds
- works near monuments
- quarrying, industrial activity, telecommunications, building works
- approval before other permits relevance

#### Articles 12-17A — Archaeological Sites and Protection Zones
Initial decoding focus:
- delimitation of archaeological sites
- temporary delimitation relevance
- zone A and zone B protection logic
- settlement-based archaeological regulation
- underwater archaeological spaces
- protection around monuments

#### Article 20 — Movable Monuments
Initial decoding focus:
- legal distinctions of movable monuments
- automatic protection
- characterization procedure
- public/private ownership relevance
- temporal thresholds

#### Articles 31-34 — Collectors, Dealers, Import/Export and Returns
Initial decoding focus:
- collectors regime
- antiquities dealers regime
- documentation duties
- legal acquisition relevance
- import/export control
- restitution and return mechanisms
- central authority relevance

#### Articles 35-44 — Archaeological Research and Protection Works
Initial decoding focus:
- archaeological fieldwork definition
- systematic excavations
- rescue excavations
- conservation and restoration works
- publication duties
- ministry authorization relevance

#### Article 45 — Museums
Initial decoding focus:
- museum establishment and operation
- legal and governance relevance
- relationship with the Ministry
- public display and institutional heritage role

#### Article 46 — Access and Use
Initial decoding focus:
- public access
- use permissions
- photography and filming relevance
- fees and conditions
- cultural use and event relevance

#### Articles 49-52 — Collective Bodies
Initial decoding focus:
- Local Councils of Monuments
- Central Archaeological Council
- Central Council of Modern Monuments
- Museum Council
- advisory governance structure

#### Articles 53-72 — Criminal Protection
Initial decoding focus:
- theft
- embezzlement
- illegal excavation
- illegal export/import
- illegal use of metal detectors
- illegal intervention on monuments
- liability of legal persons
- Prosecutor for the Protection of Cultural Goods

### C. First Decoding Logic
The first decoding pass treats Law 4858/2021 as:
- the substantive legal framework of heritage protection
- the permit/approval backbone of cultural governance
- the documentation and anti-trafficking backbone
- the sanctions and enforcement layer of the corpus

### D. Working Status Update
- decoding: in progress
- coding: pending first pass
- mapping: pending first pass
## First Coding Pass

### Articles 1-3 — Object, Definitions, and Content of Protection
**Proposed codes**
- competence
- supervision
- transition_provision

**Rationale**
These articles define the scope, public-interest logic, and substantive protection mission of the heritage regime.

### Article 4 — National Archive of Monuments
**Proposed codes**
- archive_and_documentation
- competence
- supervision

**Rationale**
Article 4 establishes the archive, documentation, registration, inspection-reporting, and controlled-access logic of the heritage system.

### Article 6 — Immovable Monuments
**Proposed codes**
- competence
- supervision
- permit_or_approval
- transition_provision

**Rationale**
Article 6 defines the classification logic of immovable monuments, the characterization procedure, and the legal effects of pending protection.

### Article 10 — Actions Affecting Monuments
**Proposed codes**
- permit_or_approval
- competence
- supervision

**Rationale**
Article 10 forms a central approval-control node for interventions, works, installations, and activity near monuments.

### Articles 12-17A — Archaeological Sites and Protection Zones
**Proposed codes**
- permit_or_approval
- competence
- supervision
- transition_provision

**Rationale**
These articles define delimitation, temporary delimitation, zone logic, settlement controls, underwater protection, and protection perimeters.

### Article 20 — Movable Monuments
**Proposed codes**
- competence
- supervision
- anti_trafficking_and_protection
- transition_provision

**Rationale**
Article 20 defines the legal structure of movable monuments and creates the baseline for later ownership, transfer, dealer, and protection regimes.

### Articles 31-34 — Collectors, Dealers, Import/Export and Returns
**Proposed codes**
- anti_trafficking_and_protection
- permit_or_approval
- supervision
- competence
- archive_and_documentation

**Rationale**
These provisions regulate collectors, dealers, declarations, import/export controls, and return/restitution mechanisms.

### Articles 35-44 — Archaeological Research and Protection Works
**Proposed codes**
- permit_or_approval
- competence
- supervision

**Rationale**
These provisions define excavation, research authorization, rescue archaeology, conservation works, and publication duties.

### Article 45 — Museums
**Proposed codes**
- museum_governance
- competence
- supervision

**Rationale**
Article 45 creates the museum-law node of the Code and links heritage protection to institutional display and operation.

### Article 46 — Access and Use
**Proposed codes**
- permit_or_approval
- competence
- supervision
- museum_governance

**Rationale**
Article 46 structures public access, use permissions, photographic/filming relevance, and controlled cultural use of monuments and spaces.

### Articles 49-52 — Collective Bodies
**Proposed codes**
- institutional_actor
- supervision
- competence

**Rationale**
These articles define the advisory governance layer through local and central councils and the Museum Council.

### Articles 53-72 — Criminal Protection
**Proposed codes**
- anti_trafficking_and_protection
- supervision
- competence

**Rationale**
These provisions form the enforcement and sanctions layer of the Code, including illegal excavation, illegal trade, illegal export, and prosecutorial protection.

## First Coding Summary
### High-frequency codes
- competence
- supervision
- permit_or_approval

### High-value specialized codes
- archive_and_documentation
- anti_trafficking_and_protection
- museum_governance
- transition_provision
- institutional_actor

## Working Status Update
- decoding: first pass completed
- coding: first pass completed
- mapping: pending first pass
