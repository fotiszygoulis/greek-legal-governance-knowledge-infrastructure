# PD 4/2018 Working Extraction v1

## Document Identity
- Document ID: DOC-001
- Title: PD 4/2018 Ministry Organization
- Working file: 2018_pd4_ministry_organization_consolidated_working_text.pdf.pdf
- Corpus folder: 01_core_normative_corpus

## Extraction Objective
This working note supports the first document-level decoding of PD 4/2018 for the AGRIR-Lab component **Greek Legal-Governance Knowledge Infrastructure for AI and Institutional Research**.

## First Extraction Targets
- ministry mission
- ministry structure
- units directly under the Minister
- internal audit function
- legislative work and parliamentary control
- strategic planning
- General Directorate of Antiquities and Cultural Heritage
- Directorates related to archives, documentation, and cultural goods protection
- Ephorates of Antiquities
- special regional services
- ministry-level governance relevance

## Coding Relevance
Primary coding categories expected:
- institutional_actor
- competence
- supervision
- internal_audit
- archive_and_documentation
- anti_trafficking_and_protection
- transition_provision

## Working Status
- decoding: not started
- coding: not started
- mapping: not started

## Note
This is the first document-level working extraction file and will be expanded gradually.
## First Decoding Pass

### A. Article-Level Priority Nodes
- Article 1: Ministry mission
- Article 2: Ministry structure
- Article 6: Internal Audit function
- Article 8: Legislative work and parliamentary control
- Article 16: General Directorate of Antiquities and Cultural Heritage
- Article 20: National Archive of Monuments and Archaeological Cadastre
- Article 21: Documentation and Protection of Cultural Goods

### B. First Institutional Extraction
#### Article 1 — Ministry Mission
Initial decoding focus:
- protection of cultural heritage
- support of contemporary cultural creation
- dissemination of cultural products
- international cultural projection
- sports-related mission elements in the original YPPOA structure

#### Article 2 — Ministry Structure
Initial decoding focus:
- units under the Minister
- internal audit
- legislative work
- strategic planning
- international and EU relations
- General Directorate layer
- Secretariat layer
- distinction between culture and sports in the original structure

#### Article 6 — Internal Audit
Initial decoding focus:
- internal control adequacy
- legality and regularity of expenditure
- economy, efficiency, effectiveness
- risk and asset management relevance
- fraud, corruption, maladministration relevance
- preliminary investigations and EDE relevance

#### Article 8 — Legislative Work and Parliamentary Control
Initial decoding focus:
- drafting and codification functions
- consultation support
- regulatory impact analysis relevance
- legal support to services
- parliamentary control coordination
- publication workflow for normative texts

#### Article 16 — General Directorate of Antiquities and Cultural Heritage
Initial decoding focus:
- central service structure
- 53 Ephorates of Antiquities
- special regional services
- museums
- national heritage governance architecture

#### Article 20 — National Archive of Monuments
Initial decoding focus:
- National Archive of Monuments
- Archaeological Cadastre
- documentation standards
- digitization and digital infrastructure
- archive governance
- image-use permissions and digital dissemination relevance

#### Article 21 — Documentation and Protection of Cultural Goods
Initial decoding focus:
- illicit trafficking of cultural goods
- restitution and recovery
- private collections and antiquities dealers
- legal documentation
- anti-trafficking governance
- metal detector registry and licensing relevance

### C. First Mapping Logic
The first mapping pass will connect:
- ministry-level units
- central directorates
- regional services
- special regional services
- documentation bodies
- protection and anti-trafficking bodies
- internal oversight nodes

### D. Working Status Update
- decoding: in progress
- coding: pending first pass
- mapping: pending first pass
## First Coding Pass

### Article 1 — Ministry Mission
**Proposed codes**
- institutional_actor
- competence
- transition_provision

**Rationale**
Article 1 defines the mission-level scope of the Ministry and preserves the original combined culture-sports logic of YPPOA.

### Article 2 — Ministry Structure
**Proposed codes**
- institutional_actor
- supervision
- competence
- transition_provision

**Rationale**
Article 2 defines the formal organizational architecture of the Ministry, including units under the Minister, secretariat layers, general directorates, and the original culture-sports structure.

### Article 6 — Internal Audit
**Proposed codes**
- internal_audit
- competence
- supervision

**Rationale**
Article 6 establishes the internal audit function as a ministry-level control, legality, expenditure-review, risk-related, and investigation-relevant node.

### Article 8 — Legislative Work and Parliamentary Control
**Proposed codes**
- competence
- supervision
- institutional_actor

**Rationale**
Article 8 defines the ministry’s internal legislative-processing, codification, consultation, parliamentary-control, and regulatory-support function.

### Article 16 — General Directorate of Antiquities and Cultural Heritage
**Proposed codes**
- institutional_actor
- competence
- supervision
- museum_governance
- transition_provision

**Rationale**
Article 16 defines the main heritage-governance architecture through central services, Ephorates of Antiquities, and special regional services.

### Article 20 — National Archive of Monuments and Archaeological Cadastre
**Proposed codes**
- archive_and_documentation
- competence
- institutional_actor
- supervision

**Rationale**
Article 20 establishes a documentation, registry, cadastre, digitization, standards, and digital-access node.

### Article 21 — Documentation and Protection of Cultural Goods
**Proposed codes**
- anti_trafficking_and_protection
- competence
- institutional_actor
- supervision
- archive_and_documentation

**Rationale**
Article 21 defines the anti-illicit-trafficking, restitution, private-collections, antiquities-dealer, documentation, and legal-protection layer.

## First Coding Summary
### High-frequency codes
- institutional_actor
- competence
- supervision

### High-value specialized codes
- internal_audit
- archive_and_documentation
- anti_trafficking_and_protection
- museum_governance
- transition_provision

## Working Status Update
- decoding: first pass completed
- coding: first pass completed
- mapping: ready for first pass
